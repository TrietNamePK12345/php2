<?php
namespace App\core;
// Quản lí các Migrations của csdl
class Database
{
    public \PDO $pdo; // \PDO : là 1 class| kiểu dữ liệu của $pdo là /PDO
    public function __construct(array $config) // đối số này ở Application
    {
        $dsn = $config['dsn'] ?? ''; // toán tử null coalescing
        $user = $config['user'] ?? '';
        $password = $config['password'] ?? '';
        $this->pdo = new \PDO($dsn, $user, $password);
        // thiết lập chế độ xử lý lỗi của PDO. Ở đây, nó được cài đặt để ném ra ngoại lệ (ERRMODE_EXCEPTION) khi có lỗi xảy ra, giúp trong quá trình phát triển và gỡ lỗi.
        $this->pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
    }

    // method này thực hiện áp dụng các migrations mới vào csdl
